...
class Geometry():
    def __init__(self, bounds, center, radius):
        ...
        self.parm_size = self.center.shape[0]
    ...
    def location(self, x, p):
        ...
        r = ((x0-self.center[p,0])**2 + (x1-self.center[p,1])**2) ** 0.5
    ...
    def intersection(self, x0, x1, p):
        ...
        idx = ((x1[:,0]-self.center[p,0])**2 + (x1[:,1]-self.center[p,1])**2) **0.5 < (self.radius+tol)
        a = (x1[:,0]-x0[:,0])**2 + (x1[:,1]-x0[:,1])**2
        b = 2*(x0[:,0]-self.center[p,0])*(x1[:,0]-x0[:,0]) + 2*(x0[:,1]-self.center[p,1])*(x1[:,1]-x0[:,1])
        c = (x0[:,0]-self.center[p,0])**2 + (x0[:,1]-self.center[p,1])**2 - self.radius**2
