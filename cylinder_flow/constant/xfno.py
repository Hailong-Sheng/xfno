...
def loss_func(Net, Conv, TrSet):
    ...
    TrSet.wei0_u0 = (TrSet.wei0_u0_diff + 
        TrSet.wei0_u0_conv_u0*TrSet.u0 + TrSet.wei0_u0_conv_u1*TrSet.u1)
    TrSet.wei1_u1 = (TrSet.wei1_u1_diff + 
        TrSet.wei1_u1_conv_u0*TrSet.u0 + TrSet.wei1_u1_conv_u1*TrSet.u1)

    TrSet.res0 = (TrSet.wei0_u0 * TrSet.u0_nb + 
                  TrSet.wei0_p * TrSet.p_nb).sum(1,keepdims=True) - TrSet.r0
    TrSet.res1 = (TrSet.wei1_u1 * TrSet.u1_nb + 
    ...
    TrSet.u0_intp = (TrSet.wei0_u0_nb * TrSet.u0_nb + 
                     TrSet.wei0_p_nb * TrSet.p_nb).sum(1,keepdims=True) + TrSet.r0_nb
    TrSet.u1_intp = (TrSet.wei1_u1_nb * TrSet.u1_nb + 
...
def train(Net, Conv, TrSet, Optim, optim_type, epochs_i): 
    ...
    if loss < loss0:
        torch.save(Net.state_dict(), './checkpoint/checkpoint_xfno.pth')
...
def solve(Net, Conv, TrSet, TeSet, Optim, Sched, optim_type, epochs_o, epochs_i):
    ...
    np.savetxt('./result/loss_history_xfno.txt', loss_history)
    np.savetxt('./result/error_u0_history_xfno.txt', error_u0_history)
    np.savetxt('./result/error_u1_history_xfno.txt', error_u1_history)
    np.savetxt('./result/error_p_history_xfno.txt', error_p_history)
...
def main():
    ...
    parser.add_argument('--fno_modes1', type=int, default=8,
                        help='fno mode1')
    parser.add_argument('--fno_modes2', type=int, default=8,
                        help='fno mode2')
    ...
    """ dataset """
    tr_set = data.TrSet(geo, msh, args.re, args.u0_inlet, dtype)
    tr_set.to(device)
    
    te_set = data.TeSet('te_set.csv', nx, dtype)
    te_set.to(device)
    '''
    ...
    """ Save results """
    net.load_state_dict(torch.load('checkpoint/checkpoint_xfno.pth'))
    te_set.u = net(te_set.parm)
    te_set.x0 = te_set.x0.reshape(-1,1)
    te_set.x1 = te_set.x1.view(-1,1)
    te_set.parm = te_set.parm.view(-1,2)
    te_set.u0 = te_set.u[:,0:1,...].reshape(te_set.nx[0]*te_set.nx[1],1)
    te_set.u1 = te_set.u[:,1:2,...].reshape(te_set.nx[0]*te_set.nx[1],1)
    te_set.p = te_set.u[:,2:3,...].reshape(te_set.nx[0]*te_set.nx[1],1)
    te_set.mask = te_set.mask.view(-1,1)
    te_set_pred = torch.cat([te_set.x0,te_set.x1,te_set.parm,
                             te_set.u0,te_set.u1,te_set.p,te_set.mask],1)
    np.savetxt('./result/te_set_pred.txt',te_set_pred.detach().cpu())
