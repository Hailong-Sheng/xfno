...
class Mesh():
    def __init__(self, geo, bounds, nx):
        ...
        self.fw_x = torch.zeros(self.c_size,self.dim)
        self.fe_x = torch.zeros(self.c_size,self.dim)
        self.fs_x = torch.zeros(self.c_size,self.dim)
        self.fn_x = torch.zeros(self.c_size,self.dim)
        self.fw_n = torch.zeros(self.c_size,self.dim)
        self.fe_n = torch.zeros(self.c_size,self.dim)
        self.fs_n = torch.zeros(self.c_size,self.dim)
        self.fn_n = torch.zeros(self.c_size,self.dim)
        ...
        self.fws_x = torch.zeros(self.c_size,self.dim)
        self.fwn_x = torch.zeros(self.c_size,self.dim)
        self.fes_x = torch.zeros(self.c_size,self.dim)
        self.fen_x = torch.zeros(self.c_size,self.dim)
        self.fws_n = torch.zeros(self.c_size,self.dim)
        self.fwn_n = torch.zeros(self.c_size,self.dim)
        self.fes_n = torch.zeros(self.c_size,self.dim)
        self.fen_n = torch.zeros(self.c_size,self.dim)
        for i in range(self.nx[0]):
            for j in range(self.nx[1]):
                m = i*self.nx[1] + j
                if self.c_loc[m]!=1:
        ...
        self.intp_v = torch.zeros(self.c_size,self.intp_n_size,3)
        for i in range(self.nx[0]):
            for j in range(self.nx[1]):
                m = i*self.nx[1] + j
                if self.c_loc[m]!=1:
